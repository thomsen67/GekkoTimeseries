...
namespace Gekko
{
    public static class Print
    {
            ...
            foreach (O.Prt.Element element in o.prtElements) //for each comma in the prt statement
            {
                ...
                for (int i = 0; i < prtElementCounter; i++)  //this element may be a #-list with 2 timeseries, x1 and x2
                {
                    ...
                    foreach (string operator2 in element.operatorsFinal)  //this may be two printcodes <n p>
                    {
                        ...
                        try
                        {                            
                            lbl = OPrintLabels(element, o.opt_label, o.opt_meta, prtElementCounter, i);
                        }
        ...
        private static void GetInfoFromMetadata(O.Prt.Element element, string opt_label, string opt_meta)
        {
            bool labelYes = G.Equal(opt_label, "yes");
            if (labelYes || opt_meta != null)
            {
                ...
                else
                {
                    foreach (char c in opt_meta)
                    {
                        ...
                        {
                            G.Warning("w12.3", "For <meta=...>, only 'n', 'l', 's', 'u', 'p' allowed, not '" + c + "'");
                        }
                    }
                }
                    ...
                    if (ts5.meta != null)
                    {
                        ...
                        for (int i = 0; i < types.Count; i++)
                        {
                            ...
                            else if (type == EMetaType.Label)
                            {
                                ...
                                string s2 = ts5.MetaGetLabel();
                                if (!G.NullOrBlanks(s2)) ss.Add(s5 + G.ReplaceWhitespaceWith1Blank(s2.Trim()));
                            }
                            else if (type == EMetaType.Source)
                            {
                                if (showType) s5 = Globals.Source + ": ";
                                string s2 = ts5.MetaGetSource();
                                if (!G.NullOrBlanks(s2)) ss.Add(s5 + G.ReplaceWhitespaceWith1Blank(s2.Trim()));
                            }
                            else if (type == EMetaType.Units)
                            {
                                if (showType) s5 = Globals.Units + ": ";
                                string s2 = ts5.MetaGetUnits();
                                if (!G.NullOrBlanks(s2)) ss.Add(s5 + G.ReplaceWhitespaceWith1Blank(s2.Trim()));
                            }
                            else if (type == EMetaType.Stamp)
                            {
                                if (showType) s5 = Globals.Stamp + ": ";
                                string s2 = ts5.MetaGetStamp();
                                if (!G.NullOrBlanks(s2)) ss.Add(s5 + G.ReplaceWhitespaceWith1Blank(s2.Trim()));
                            }
                        }
        ...
        private static Table PrintMixedMD(GekkoSmpl smpl, EPrintTypes type, bool rows, List<O.Prt.Element> containerExplode, int labelMaxLine, int n, bool pretty, GekkoDictionary<string, bool> freqs, O.Prt o)
        {
            ...
            for (int j = 1; j < n + 2; j++)
            {
                ...
                Series tsWork, tsRef;
                PrintPrepareColumn(smpl, type, containerExplode, j, o, out cc, out operator2, out label, out format, out freqColumn, out scalarValueWork, out tsWork, out scalarValueRef, out tsRef);
                    ...
                    foreach (GekkoTime t in new GekkoTimeIterator(Program.ConvertFreqs(smpl.t1, smpl.t2, EFreq.D)))  //handles if the freq given is not daily
                    {
                        ...
                        if (counter == 1 || t.subsub == 1)
                        {
                            ...
                            else
                            {
                                ...
                                {                                    
                                    ...
                                    {
                                        d = PrintHelperTransformScalar(scalarValueWork, scalarValueRef, operator2, o.guiGraphIsLogTransform, o.opt_yoy, o.opt_i, EPrtCollapseTypes.None, 1, skipCounter);
                                    }
                                    else
                                    {
                                        d = PrintHelperTransform(smpl, tsWork, tsRef, tMonth, operator2, o.guiGraphIsLogTransform, o.opt_yoy, o.opt_i, EPrtCollapseTypes.None, 1, skipCounter);
                                    }
                        ...
                        else
                        {
                            ...
                            {                                
                                ...
                                {
                                    d = PrintHelperTransform(smpl, tsWork, tsRef, t, operator2, o.guiGraphIsLogTransform, o.opt_yoy, o.opt_i, EPrtCollapseTypes.None, 1, skipCounter);
                                }
                        ...
                        if (counter == 1 || dt.DayOfWeek == DayOfWeek.Monday)
                        {
                            ...
                            else
                            {
                                if (dt.DayOfWeek == DayOfWeek.Monday && isWeeklyFreq)
                                {
                                    ...
                                    {
                                        d = PrintHelperTransform(smpl, tsWork, tsRef, tWeek, operator2, o.guiGraphIsLogTransform, o.opt_yoy, o.opt_i, EPrtCollapseTypes.None, 1, skipCounter);
                                    }
                                    table.SetNumber(i, j, d, format);
                                }
                            }
        ...
        public static void PrintPrepareColumn(GekkoSmpl smpl, EPrintTypes type, List<O.Prt.Element> containerExplode, int j, O.Prt o, out O.Prt.Element cc, out string operator2, out List<string> label, out string format, out EFreq freqColumn, out double scalarValueWork, out Series tsWork, out double scalarValueRef, out Series tsRef)
        {
            cc = null;
            ...
            if (ivWork != null)
            {
                ...
                if (tsWork == null)
                {
                    ...
                    if (o.scaleCode != null)
                    {                        
                        tsWork = ScaleVal(smpl, o.scaleCode, 0, scalarValueWork, o);
                    }
                }
                else
                {                    
                    if (o.scaleCode != null)
                    {
                        tsWork = ScaleSeries(smpl, o.scaleCode, 0, tsWork, o);
                    }
                }
            }
            ...
            if (ivRef != null)
            {
                ...
                if (tsRef == null)
                {
                    ...
                    if (o.scaleCode != null)
                    {
                        tsRef = ScaleVal(smpl, o.scaleCode, 1, scalarValueRef, o);
                    }
                }
                else
                {
                    if (o.scaleCode != null)
                    {
                        tsRef = ScaleSeries(smpl, o.scaleCode, 1, tsRef, o);
                    }
                }
            }
        }

        private static Series ScaleSeries(GekkoSmpl smpl, string scaleCode, int bankNumber, Series ts, O.Prt o)
        {
            //TODO: freq mismatch            
            ...
            {
                scale = o.scale[bankNumber]; //already done
            }
        ...
        public static double PrintHelperTransform(GekkoSmpl smpl, Series tsWork, Series tsRef, GekkoTime t, string operator2, bool logTransform, string isYoy, GekkoTime index, EPrtCollapseTypes collapse, int sumOver, int[] skipCounter)
        {
            ...
            Program.ComputeValueForPrintPlotNew(smpl, out var1, out varPch, operator2, t, tsWork, tsRef, logTransform, isYoy, index, false, collapse, sumOver);
            return var1;
        }
