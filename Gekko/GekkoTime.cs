...
namespace Gekko
{
    ...
    public enum EFreq 
    {
        ...
        None = 4,     //used to signal non-freq variable (usually something failing or not assigned, like "null")
        D = 5,        //daily        
        W = 6,        //weekly
        Lag = 7,      //used for time-lags, where the .super short can be negative.
        Age = 8,      //used for non-time short integers like age and the like, which can be calculated on (like x[18+1, 2020+1] for the cohort after the 18yo in 2020 (x[18, 2020]).
        Empty5 = 9,   // --------> this and the following can be filled/changed
        Empty6 = 10,
        Empty7 = 11,
        Empty8 = 12,
        Empty9 = 13,
    ...
    public struct GekkoTime : IComparable<GekkoTime>
    {
        ...
        //For Gekko 4.0: use EFreq.None for tNull and tSimilarToNull, so they will not be accepted for any time iterator!
        ...
        public static GekkoTime tSimilarToNull = new GekkoTime(EFreq.A, -23456, 1);  //DO NOT CHANGE NAME! Think of it as 'yes' (for instance in PRT <i>... which is different from PRT <i=2010q3>).
        ...
        {            
            ...
            if (Program.options.bugfix_dates_fast && !allowTwoDigits)
            {
                ...
                else if (c4 == 'm' || c4 == 'M')
                {
                    ...
                    if (i_d != -12345)
                    {
                        ...
                        return new GekkoTime(EFreq.D, y, i2, i3);
                    }
                    else
                    {
                        int i2 = G.IntParse(s.Substring(i4 + 1, s.Length - i4 - 1));
            ...
            //
            // The following is quite slow, also because it seems date consistency ends up being checked two times.
            //
