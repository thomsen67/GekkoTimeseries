...
namespace Gekko
{
    public class Translators
    {
        ...
        private static string HandleUpd(string textInput)
        {
            ...
            {
                xx[i] = xx[i].TrimEnd();
                int begin = FindFirstNonBlank(xx[i]);
                string ss = " "; if (begin != -12345) ss = (xx[i] + "    ").Substring(begin, 4);
                if (ss.ToLower() == "upd ")
                {
                    isUpd = true;
                    continue;
                }
                string ss2 = " "; if (begin != -12345) ss2 = xx[i].Substring(begin, 1);
                if (ss2.ToLower() == "-" || ss2.ToLower() == "." || char.IsDigit((char)ss2.ToLower()[0]))
                {
                    //number starting with - or . or digit
                    if (isUpd)
                    {
                        if (!xx[i - 1].Contains("&")) xx[i - 1] += " &";
                    }
                }
                else isUpd = false;
            }
            ...
            {
                string op = "+*%^=#";
                if (xx[i].ToLower().Contains("upd "))
                {
                    foreach (char c in op)
                    {
                        string s5 = c.ToString();
                        xx[i] = xx[i].Replace(s5 + "$", s5 + " __dollar__ ");
                        xx[i] = xx[i].Replace(s5 + " $", s5 + "  __dollar__ ");
                        xx[i] = xx[i].Replace(s5 + "  $", s5 + "   __dollar__ ");
                        xx[i] = xx[i].Replace(s5 + "   $", s5 + "    __dollar__ ");
                    }
            ...
            foreach (string s in xx) s3 += s + G.NL;
            return s3;
        }
