...
{    
    ...
    public static class SolveCommon
    {
        ...
        {
            return Program.model.modelGekko.varsDTypeAutoGenerated.ContainsKey(new DName(variable)) || Program.model.modelGekko.varsJTypeAutoGenerated.ContainsKey(new DName(variable)) || Program.model.modelGekko.varsZTypeAutoGenerated.ContainsKey(new DName(variable));
        }
        ...
        public static void InitEndoLaggedOrExo(int[] extraWritebackPointers, int[] lagPointers, int[] endoPointers, int[] isDJZvarPointers, double[,] a, int tInt, ref GekkoTime t, ref GekkoTime tStart, ref GekkoTime tEnd, int i, ref double val, Series ts, string variable, int yy, YesNoNull isStaticLocalOption)
        {
            ...
            if (double.IsNaN(val))
            {
                ...
                else
                {
                    ...
                    GekkoDictionary<string, string> zvars = new GekkoDictionary<string, string>(StringComparer.OrdinalIgnoreCase);
                    try
                    {                        
                        foreach (DName zname in Program.model.modelGekko.varsZTypeAutoGenerated.Keys)
                        {
                            string dname = "D" + zname.ToString().Substring(1);
                            if (Program.model.modelGekko.varsDTypeAutoGenerated.ContainsKey(new DName(dname)))
                            {
                                //We have a dabc and a zabc. Now we check if dabc == 1 while zabc is missing.
                                Series d_series = Program.databanks.GetFirst().GetIVariable(G.Chop_SetFreq(dname, Program.options.freq)) as Series;
                                Series z_series = Program.databanks.GetFirst().GetIVariable(G.Chop_SetFreq(zname.ToString(), Program.options.freq)) as Series;
                                if (d_series != null && z_series != null)
                                {
                                    foreach (GekkoTime gt in new GekkoTimeIterator(tStart, tEnd))
                                    {
                                        ...
                                        {
                                            if (!zvars.ContainsKey(zname.ToString())) zvars.Add(zname.ToString(), null);
                                        }
                                    }
                                }
                            }
                    ...
                    if (zvars.Count > 0)
                    {
                        G.Writeln("    Beware that the following active Z-variables contain missing values:");
                        G.Writeln("    " + Stringlist.GetListWithCommas(zvars.Keys.ToArray()));
                    }
        ...
        public static void Sim(O.Sim o)
        {
            ...
            if (!G.IsUnitTestingOrNotShowingGUI()) Gekko.Gui.gui.textBoxMainTabUpper.SuspendLayout();
            ...
            if (!G.IsUnitTestingOrNotShowingGUI()) Gekko.Gui.gui.textBoxMainTabUpper.ResumeLayout();
        }        
        ...
        public static void SimFast(GekkoTime tStart, GekkoTime tEnd, SimOptions so, P p)
        {
            ...
            if (Program.options.solve_gauss_conv_ignorevars == true)
            {
                foreach (string var in Globals.checkoff)
                {
                    ...
                    if (Program.model.modelGekko.varsBType.ContainsKey(new DName(var + Globals.lagIndicator + "0")))
                    {
                        int bNumber = Program.model.modelGekko.varsBType[new DName(var + Globals.lagIndicator + "0")].bNumber;
                        if (!checkoff.ContainsKey(bNumber)) checkoff.Add(bNumber, 0);
                    }
                }
            }

            if (!(G.Equal(so.method, "gauss") || G.Equal(so.method, "newton") || G.Equal(so.method, "res") || G.Equal(so.method, "reverted") || G.Equal(so.method, "eigen"))) G.Warning("w24.2", "Seems to be a problem with model type");
            ...
            if (Program.options.solve_gauss_dump)
            {
                ...
                GC.Collect();
                Program.model.modelGekko.bMemory = new GekkoDictionary<GekkoTime, List<IterMemory>>();
            }
            ...
            foreach (BTypeData value in Program.model.modelGekko.varsBType.Values)
            {
                ...
                if (value.lag == 0 && Program.model.modelGekko.reverted.ContainsKey(new DName(value.variable)))
                {
                    revertedPointers[value.bNumber] = 1;
                }
                //for now, we ignore what the value may be (all damping is with same factor)
                if (Program.model.modelGekko.dampVariables.ContainsKey(new DName(value.variable + Globals.lagIndicator + value.lag)))  //with lag indicator
                {
                ...
                foreach (NewtonFairTaylorHelper1 shock in helper.shocks)
                {
                    ...
                    if (usingFairTaylor || usingNewtonFairTaylor)
                    {
                        if (shock.isFirstBaseline)
                        {
                            ...
                            if (ft == ftMax - 1)
                            {
                                G.Warning("w25.1", extra + "Fair-Taylor algorithm did not converge in " + (ft + 1) + " " + extra2 + "FT-iterations (" + G.SecondsFormat((DateTime.Now - dtFt).TotalMilliseconds) + ")");
                                a = aFinalResultFromShockLoop;
                                goto JumpOut;
                            }
                        }
                ...
                else
                {
                    if (Globals.simulationCheckThatAllDataGetsFromBArrayToTimeSeries)
                    {
                        if (Program.model.modelGekko.b[i] != bCheck[i])  //probably is false if left side is 0 and right side is NaN. Not good.
                        {
                            ...
                            string var = Program.model.modelGekko.varsBTypeInverted[i].ToString();
    ...
    public static class SolveGauss777
    {
        ...
        public static void SolveGauss(bool ftOrNft, double[] b, List<int> isDampedPointers, int[] isDampedPointersArray, out string culprit, ECompiledModelType modelType, GekkoTime t, Dictionary<int, int> checkoff)
        {
            ...
            if (Program.options.solve_gauss_dump)
            {
                ...
                Program.model.modelGekko.bMemory.Add(t, iterMemories);
            }
            ...
            BTypeData ti = null; Program.model.modelGekko.varsBType.TryGetValue(new DName(var + Globals.lagIndicator + "0"), out ti);
            if (ti == null)
            {
                    ...
                    for (int it = 0; it < int.MaxValue; it++)
                    {
                        ...
                        if (Globals.runningOnTTComputer)
                        {
                            ...
                            if (largest >= 0)
                            {
                                ...
                                string var = Program.model.modelGekko.varsBTypeInverted[bnum].ToString();
                                string var2 = "";
                                int lag = 0;
                ...
                if (Program.model.modelGekko.largestLeadOutsideRevertedPart > 0)
                {
                    ...
                    foreach (BTypeData data in Program.model.modelGekko.varsBType.Values)
                    {
                        ...
                        for (int i = 0; i < Program.model.modelGekko.largestLeadOutsideRevertedPart; i++)
                        {
                            ...
                            Program.model.modelGekko.varsBType.TryGetValue(new DName(data.variable + Globals.lagIndicator + i), out data2);
    ...
    public static class SolveOrdering
    {
        ...
        private static int FindEqWithVarOnLeftHandSide(string endo)
        {
            ...
            if (eqEndo == -12345) G.Warning("w25.2", "Variable " + endo + " is not found as left-hand side var");
            return eqEndo;
        }
        ...
        public static void EndogenizeExogenizeStuff(bool isFix)
        {
            ...
            foreach (DName var in Program.model.modelGekko.endogenousOriginallyInModel.Keys)
            {
                Program.model.modelGekko.m2.endogenous.Add(var.ToString(), "");
            }
            ...
            if (isFix)  //we skip this for normal SIM, just as if the ENDO/EXO lists were empty even if they are not
            {
                ...
                for (int i = 0; i < endogeni.Count; i++)
                {
                    ...
                    BTypeData ss1 = null; Program.model.modelGekko.varsBType.TryGetValue(new DName(s1 + Globals.lagIndicator + "0"), out ss1);
                    if (ss1 == null)
                    {
                    ...
                    else
                    {
                        ...
                        BTypeData ss2 = null; Program.model.modelGekko.varsBType.TryGetValue(new DName(s2 + Globals.lagIndicator + "0"), out ss2);
                        if (ss2 == null)
                        {
                            //TODO: general error handling regarding endo/exo
                            //now we get runtime error
            ...
            foreach (EquationHelper eh in Program.model.modelGekko.equations)
            {
                ...
                {
                    BTypeData temp3 = (BTypeData)Program.model.modelGekko.varsBType[new DName(s)];
        ...
        public static void FeedbackOrderingStuff(ECompiledModelType modelType, bool isCalledFromModelStatement)
        {
            ...
            foreach (string endo in Program.model.modelGekko.m2.endoSubstitution.Keys)
            {
                ...
                int exoBtype = Program.model.modelGekko.varsBType[new DName(exo + Globals.lagIndicator + "0")].bNumber;
                int endoBtype = Program.model.modelGekko.varsBType[new DName(endo + Globals.lagIndicator + "0")].bNumber;
                ...
                foreach (int eq in eqsRhsExo)
                {
                    ...
                    {
                        int number = al[i];
                        if (number == exoBtype) G.Warning("w25.2", "Internal error #32108743");
                    }
                    al.Add(exoBtype);
                }
            ...
            if (!(simulEpi.Count == 0))
            {
                G.Warning("w25.2", "Error in feedback/recursive");
            }
