        ...
        public static void InitEndoLaggedOrExo(int[] extraWritebackPointers, int[] lagPointers, int[] endoPointers, int[] isDJZvarPointers, double[,] a, int tInt, ref GekkoTime t, ref GekkoTime tStart, ref GekkoTime tEnd, int i, ref double val, Series ts, string variable, int yy, YesNoNull isStaticLocalOption)
        {
            ...
            if (double.IsNaN(val))
            {
                ...
                else
                {
                    ...
                    try
                    {                        
                        foreach (string zname in Program.model.modelGekko.varsZTypeAutoGenerated.Keys)
                        {
                            string dname = "D" + zname.Substring(1);
                            if (Program.model.modelGekko.varsDTypeAutoGenerated.ContainsKey(dname))
                            {
                                ...
                                Series z_series = Program.databanks.GetFirst().GetIVariable(G.Chop_SetFreq(zname, Program.options.freq)) as Series;
