...
{    
    ...
    public static class SolveCommon
    {
        ...
        {
            return Program.model.modelGekko.varsDTypeAutoGenerated.ContainsKey(new DNameSimplest(variable)) || Program.model.modelGekko.varsJTypeAutoGenerated.ContainsKey(new DNameSimplest(variable)) || Program.model.modelGekko.varsZTypeAutoGenerated.ContainsKey(new DNameSimplest(variable));
        }
                ...
                else
                {
                    ...
                    {                        
                        foreach (DName zname in Program.model.modelGekko.varsZTypeAutoGenerated.Keys)
                        {
                            ...
                            if (Program.model.modelGekko.varsDTypeAutoGenerated.ContainsKey(new DNameSimplest(dname)))
                            {
        ...
        public static void SimFast(GekkoTime tStart, GekkoTime tEnd, SimOptions so, P p)
        {
            ...
            if (Program.options.solve_gauss_conv_ignorevars == true)
            {
                foreach (string var in Globals.checkoff)
                {
                    ...
                    if (Program.model.modelGekko.varsBType.ContainsKey(new DNameSimplest(var + Globals.lagIndicator + "0")))
                    {
                        int bNumber = Program.model.modelGekko.varsBType[new DNameSimplest(var + Globals.lagIndicator + "0")].bNumber;
            ...
            foreach (BTypeData value in Program.model.modelGekko.varsBType.Values)
            {
                ...
                if (value.lag == 0 && Program.model.modelGekko.reverted.ContainsKey(new DNameSimplest(value.variable)))
                {
                    revertedPointers[value.bNumber] = 1;
                }
                //for now, we ignore what the value may be (all damping is with same factor)
                if (Program.model.modelGekko.dampVariables.ContainsKey(new DNameSimplest(value.variable + Globals.lagIndicator + value.lag)))  //with lag indicator
                {
    ...
    public static class SolveGauss777
    {
        ...
        public static void SolveGauss(bool ftOrNft, double[] b, List<int> isDampedPointers, int[] isDampedPointersArray, out string culprit, ECompiledModelType modelType, GekkoTime t, Dictionary<int, int> checkoff)
        {
            ...
            BTypeData ti = null; Program.model.modelGekko.varsBType.TryGetValue(new DNameSimplest(var + Globals.lagIndicator + "0"), out ti);
            if (ti == null)
            {
                ...
                if (Program.model.modelGekko.largestLeadOutsideRevertedPart > 0)
                {
                    ...
                    foreach (BTypeData data in Program.model.modelGekko.varsBType.Values)
                    {
                        ...
                        for (int i = 0; i < Program.model.modelGekko.largestLeadOutsideRevertedPart; i++)
                        {
                            ...
                            Program.model.modelGekko.varsBType.TryGetValue(new DNameSimplest(data.variable + Globals.lagIndicator + i), out data2);
    ...
    public static class SolveOrdering
    {
        ...
        public static void EndogenizeExogenizeStuff(bool isFix)
        {
            ...
            foreach (EquationHelper eh in Program.model.modelGekko.equations)
            {
                ...
                {
                    BTypeData temp3 = (BTypeData)Program.model.modelGekko.varsBType[new DNameSimplest(s)];
        ...
        public static void FeedbackOrderingStuff(ECompiledModelType modelType, bool isCalledFromModelStatement)
        {
            ...
            foreach (string endo in Program.model.modelGekko.m2.endoSubstitution.Keys)
            {
                ...
                int exoBtype = Program.model.modelGekko.varsBType[new DNameSimplest(exo + Globals.lagIndicator + "0")].bNumber;
                int endoBtype = Program.model.modelGekko.varsBType[new DNameSimplest(endo + Globals.lagIndicator + "0")].bNumber;
