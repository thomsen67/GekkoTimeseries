...
namespace Gekko
{
    ...
    public class GekkoPivotTable
    {
        ...
        private static string GekkoPivotGroup(List<int> selectedIndexes, Func<FrameLightRow, int, string> group, FrameLightRow row, int lhsFrameCol, DecompOptions2 decompOptions2)
        {
            ...
            int iVars; if (!row.parent.frameDimensionNames.TryGetValue(Globals.col_variable, out iVars)) iVars = -12345;
            if (selectedIndexes.Contains(iVars)) hasVarsSelected = true;

            bool hasTimeSelected = false;
            int iTime; if (!row.parent.frameDimensionNames.TryGetValue(Globals.col_t, out iTime)) iTime = -12345;
            ...
            if (decompOptions2.useBracketNames && hasVarsSelected && !hasTimeSelected)
            {
                ...
                int iLags; if (!row.parent.frameDimensionNames.TryGetValue(Globals.col_lag, out iLags)) iLags = -12345;
    ...
    public class Decomp
    {
                ...
                else
                {
                    ...
                    foreach (DName s in decompOptions2.new_from)
                    {
                        ...
                        else
                        {
                            ...
                            link.varnames = null; //empty. Old: "<not used>"; //strange but necessary further on
                        }
        ...
        public static void PrepareEquations(GekkoTime per1, GekkoTime per2, DecompOperator operator1, DecompOptions2 decompOptions2, bool showErrors, ModelGamsScalar modelGamsScalar)
        {
            ...
            foreach (DName s in decompOptions2.new_from)
            {
                ...
                MultidimElement mmi = new MultidimElement(s.GetIndexesExceptTime().Select(x => x.GetString()).ToArray());
                ...
                FindEquationsForEachRelevantPeriod(per1, per2, s.RemoveTime(), new DNameSimplest(s.GetName()), mmi, element, operator1, showErrors, modelGamsScalar);
            }
                    ...
                    foreach (DecompStartHelper eqPeriods in link.GAMS_dsh)  //unrolling: for each uncontrolled #i in x[#i]
                    {
                        ...
                        foreach (GekkoTime t in new GekkoTimeIterator(per1, per2))
                        {
                            ...
                            foreach (long dp in modelGamsScalar.precedents[eqNumber].vars)
                            {
                                ...
                                DName varName = modelGamsScalar.GetVarNameA(ModelGamsScalar.UnpackVariable(dp));

                                int add2 = 0;

                                int date = ModelGamsScalar.UnpackPeriod(dp);
                                int tt1 = date + add + add2;
                                int tt2 = date + add + add2 - t.Subtract(modelGamsScalar.tBasis);

                                if (modelGamsScalar.isTimeless[ModelGamsScalar.UnpackVariable(dp)])
                                {
                            ...
                            foreach (TwoDNames two in variables)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        int col; if (!endo.TryGetValue(dnX1, out col)) col = -12345;
                                        if (!(row < mEndo.GetLength(0) && col < mEndo.GetLength(1)))
                                        {
                                            new Error("DECOMP matrix invert problem");
                                        }
                                    ...
                                    {
                                        int col;if (!exo.TryGetValue(dnX1, out col)) col = -12345;
                                        if (!(row < mExo.GetLength(0) && col < mExo.GetLength(1)))
                                        {
                                            new Error("DECOMP matrix invert problem");
                                        }
            ...
            foreach (GekkoTime t in new GekkoTimeIterator(gt1, gt2))
            {
                ...
                {                                        
                    ...
                    if (op.isRaw)
                    {
                        ...
                        double x0 = modelGamsScalar.GetData(ModelGamsScalar.UnpackPeriod(dp), tZero, ModelGamsScalar.UnpackVariable(dp), missingAsZero, true);
                        double x1 = modelGamsScalar.GetData(ModelGamsScalar.UnpackPeriod(dp), tZero, ModelGamsScalar.UnpackVariable(dp), missingAsZero, false);
                        int lag2 = ModelGamsScalar.UnpackPeriod(dp) + timeIndex2;
                        if (modelGamsScalar.isTimeless[ModelGamsScalar.UnpackVariable(dp)])
                        {
                            lag2 = 0;
                        }
                    ...
                    else
                    {
                        if (op.lowLevel == ELowLevel.OnlyQuo || op.lowLevel == ELowLevel.BothQuoAndRef)
                        {
                            ...
                            double x0_before = modelGamsScalar.GetData(ModelGamsScalar.UnpackPeriod(dp), tZero, ModelGamsScalar.UnpackVariable(dp), missingAsZero, false);
                            double x1 = modelGamsScalar.GetData(ModelGamsScalar.UnpackPeriod(dp) + 1, tZero, ModelGamsScalar.UnpackVariable(dp), missingAsZero, false);

                            try
                            {
                                double x0_after = x0_before + eps;                                
                                modelGamsScalar.SetData(ModelGamsScalar.UnpackPeriod(dp), tZero, ModelGamsScalar.UnpackVariable(dp), false, x0_after);                                
                            ...
                            {
                                modelGamsScalar.SetData(ModelGamsScalar.UnpackPeriod(dp), tZero, ModelGamsScalar.UnpackVariable(dp), false, x0_before);
                            }
                        }
                        ...
                        if (op.lowLevel == ELowLevel.OnlyRef || op.lowLevel == ELowLevel.BothQuoAndRef)
                        {
                            ...
                            double x0_before = modelGamsScalar.GetData(ModelGamsScalar.UnpackPeriod(dp), tZero, ModelGamsScalar.UnpackVariable(dp), missingAsZero, true);
                            double x1 = modelGamsScalar.GetData(ModelGamsScalar.UnpackPeriod(dp) + 1, tZero, ModelGamsScalar.UnpackVariable(dp), missingAsZero, true);

                            try
                            {
                                double x0_after = x0_before + eps;
                                modelGamsScalar.SetData(ModelGamsScalar.UnpackPeriod(dp), tZero, ModelGamsScalar.UnpackVariable(dp), true, x0_after);
                            ...
                            {
                                modelGamsScalar.SetData(ModelGamsScalar.UnpackPeriod(dp), tZero, ModelGamsScalar.UnpackVariable(dp), true, x0_before);
                            }
                        }
        ...
        private static FrameLight DecompPivotCreateDataframe(GekkoSmpl smpl, GekkoTime per1, GekkoTime per2, DName lhs, DName lhs2, DecompData decompDataMAINClone, DecompDatas decompDatas, DecompOperator op, EContribType operatorOneOf3Types, DecompOptions2 decompOptions2, Model model)
        {
            ...
            foreach (GekkoTime t2 in new GekkoTimeIterator(per1, per2))
            {
                ...
                foreach (DName fullVariableName in orderedNames)
                {
                    ...
                    List<string> indexes = chop.fullName.GetIndexesExceptTime().Select(x => x.ToString()).ToList();
        ...
        {
            string[] domains = null;
            List<string> indexes = fullName.GetIndexesExceptTime().Select(x => x.ToString()).ToList();
            if (indexes != null)
            {
        ...
        private static string ReplaceStars(List<DName> vars_hack, string s)
        {
            ...
            {
                List<string> xx = var.GetIndexesExceptTime().Select(x => x.ToString()).ToList();
                Dims dims = new Dims();
        ...
        private static void FindConnection(GekkoTime t0, string x1, string x2, ModelGamsScalar modelGamsScalar)
        {
            ...
            long pv2 = ModelGamsScalar.PackPeriodAndVariable(timeIndex, a2);            
