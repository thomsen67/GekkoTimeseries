...
Func<IVariable> func17 = () => {
var smplCommandRemember18 = smpl.command; smpl.command = GekkoSmplCommand.Unfold;
List temp16 = new List();

foreach (IVariable listloop_a15 in new O.GekkoListIterator(O.Lookup(smpl, null, ((O.scalarStringHash).Add(smpl, (new ScalarString("a")))), null, false, EVariableType.Var))) {
O.ClearLabelHelper(smpl);
temp16.Add(O.Lookup(smpl, null, (O.ReportInterior(smpl, O.Add(smpl, O.Lookup(smpl, null, null, "#a", null, null, false, EVariableType.Var), new ScalarString(ScalarString.SubstituteScalarsInString(@"z", true, false))), 0, labelCounter)), null, false, EVariableType.Var));

O.AddLabelHelper(smpl);
}
smpl.command = smplCommandRemember18;
return temp16;
...
ope0.variable[bankNumber] = func17();
