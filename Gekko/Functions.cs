...
namespace Gekko
{
    ...
    public class Functions
    {
        ...
        public static void versioning(GekkoSmpl smpl, IVariable _t1, IVariable _t2, params IVariable[] input)
        {
            ...
            //System.Windows.Forms.MessageBox.Show(Program.options.databank_versioning_name + " known names: " + Stringlist.GetListWithCommas(dlinkFiles));
            ...
            foreach (string dlinkFile in dlinkFiles) //Could probably be parallelized
            {
                ...
                // --------------------------------------------------------------------------------------------------
                bool isDataFileOk = Helper_versioningIsDataFileOk(dataFile, blobInfo, syncStampFile);
                if (isDataFileOk)
                {
                    //Check that we have the file in blobs folder, else add it
                    Program.BlobsFile(dataFile, blobInfo.sha256, blobsFolder, false);
                }
                else 
                {
                    //Get it from blobs (A or B)
                    Program.BlobsFile(dataFile, blobInfo.sha256, blobsFolder, true);
                }
            }

            //System.Windows.Forms.MessageBox.Show("Gekko: versioning(): --- last sync: " + syncTimeUtc.ToString());
            Stamp stamp2 = new Stamp() { stamp = DateTime.UtcNow };
            G.YamlWriter<Stamp>(stamp2, syncStampFile);
        }

        /// <summary>
        /// Returns true if file must be fetched from blobs
        /// </summary>
        ...
        /// <param name="fi"></param>
        /// <returns></returns>
        private static bool Helper_versioningIsDataFileOk(string dataFile, BlobInfo blobInfo, string syncStampFile)
        {
            ...
            {
                //Will return true: if the file (that exists with the right size) has not changed since a .dlink file was last created, we consider it ok (we give 2 s slack)            
            }
            else
            {
