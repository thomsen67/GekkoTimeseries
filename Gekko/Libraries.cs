        ...
        public void LoadLibraryFromZip(O.Library o)
        {
            for (int i = 0; i < o.files.Count; i++)
            {
                ...
                else
                {
                    ...
                    string libHash = Program.GetMD5Hash(ss, null, null, null); //Pretty unlikely that two different libs could produce the same hash.
            ...
            using (ZipArchive archive = ZipFile.OpenRead(fileNameWithPath))
            {
                foreach (ZipArchiveEntry entry in archive.Entries)
                {
                    ...
                    else
                    {
                        ...
                        {                            
                            string tempFileNameWithPath = Program.WaitForZipExtractFileEntryToTempFile(entry, fileNameWithPath);
