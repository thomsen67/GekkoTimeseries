...
namespace Gekko.Parser.Frm
{
    class ParserFrmWalkAST
    {
        ...
        public static void ParserFrmWalkASTHelper(GekkoDictionary<string, string> vals)
        {
            ...
            foreach (EquationHelper eh in Program.model.modelGekko.equations)
            {
                if (Program.model.modelGekko.endogenousOriginallyInModel.ContainsKey(new DNameSimplest(eh.lhs)))
                {
                    new Error("When finding endogenous: it seems variable '" + eh.lhs.ToString() + "' appears multiple times on left-hand side");
                }
                else
                {
                    Program.model.modelGekko.endogenousOriginallyInModel.Add(new DNameSimplest(eh.lhs), null);
                    Program.model.modelGekko.endogenousBNumbersOriginallyInModel.Add(eh.bNumberLhs, null);
                }
            ...
            foreach (EquationHelper eh in Program.model.modelGekko.equations)
            {
                Program.model.modelGekko.fromVariableToEquationNumber.Add(new DNameSimplest(eh.lhsWithLagIndicator), eh.equationNumber);
            }

            foreach (EquationHelper eh in Program.model.modelGekko.equationsReverted)
            {
                if (Program.model.modelGekko.reverted.ContainsKey(new DNameSimplest(eh.lhs)))
                {
                ...
                {
                    Program.model.modelGekko.reverted.Add(new DNameSimplest(eh.lhs), null);
                }
            ...
            foreach (EquationHelper eh in Program.model.modelGekko.equations)
            {
                foreach (string p in eh.precedentsWithLagIndicator.Keys)
                {
                    ...
                    if (Program.model.modelGekko.dependents.ContainsKey(new DNameSimplest(p2)))
                    {
                    }
                    else
                    {
                        DependentsHelper d = new DependentsHelper();
                        Program.model.modelGekko.dependents.Add(new DNameSimplest(p2), d);
                    }
                    Dictionary<DName, DName> yy = Program.model.modelGekko.dependents[new DNameSimplest(p2)].storage;
                    if (!yy.ContainsKey(new DNameSimplest(eh.lhs)))
                    {
                        yy.Add(new DNameSimplest(eh.lhs), null);
                    }
                }
            }
        ...
        private static void AddToDictionary(Dictionary<DName, DName> ht, string name, ModelGekko model)
        {
            if (ht.ContainsKey(new DNameSimplest(name)))
            {
                model.fatalEndogenousError = true;
            }
            else
            {
                ht.Add(new DNameSimplest(name), null);
            }
        }
        ...
        private static void ExtractDJZAndEquationTypeFromEquationCode(EquationHelper eh, ModelGekko model, bool isModel)
        {
            ...
            if (isModel && code.Length >= 7)  //damping not relevant if not isModel (i.e. a GENR statement)
            {
                ...
                if (G.Equal(eh.equationCodeZ, "Z"))
                {
                    ...
                    if (model.dampVariables.ContainsKey(new DNameSimplest(var)))
                    {
                        G.Warning("w24.4", "It seems the same damp variable appears multiple times");
                    }
                    else
                    {
                        model.dampVariables.Add(new DNameSimplest(var), null);
                    }
                }
            }
            return;
        ...
        private static void PrintVariableAsBType(EquationHelper eh, WalkerHelper2 wh2, ModelGekko model, string variable, int lag, bool leftSide, bool humanReadable, bool isModel, bool isBaseBank, string absoluteTime, string namedBank)
        {
            ...
            if (isModel)  //for equations in model/frm, probably always true nowadays
            {
                ...
                ATypeData dataA = null; model.varsAType.TryGetValue(new DNameSimplest(variable), out dataA);
                ...
                if (dataA == null)
                {
                    ...
                    model.varsAType.Add(new DNameSimplest(variable), dataA);
                }
                ...
                BTypeData data = null; model.varsBType.TryGetValue(new DNameSimplest(input), out data);
                if (data == null)
                {
                    data = new BTypeData();
                    ...
                    model.varsBType.Add(new DNameSimplest(input), data);
                    model.varsBTypeInverted.Add(data.bNumber, new DNameSimplest(input));
                }
