...
namespace Gekko.Parser.Frm
{
    class ParserFrmCompileAST
    {
        ...
        private static string GetCacheKey(bool isFix)
        {
            ...
            if (isFix) foreach (DName s in Program.model.modelGekko.endogenized.Keys) temp1.Add(s.ToString().ToLower());
            List<string> temp2 = new List<string>();
            if (isFix) foreach (DName s in Program.model.modelGekko.exogenized.Keys) temp2.Add(s.ToString().ToLower());
            temp1.Sort();
            temp2.Sort();
        ...
        private static void PrintInfoFilesCreateVarsEtc(bool isCalledFromModelStatement)
        {
            ...
            if (true)
            {
                List<DName> al = Program.model.modelGekko.varsAType.Keys.ToList();
                al= al.OrderBy(x => x, new MultidimSortComparer(true)).ToList();
                ...
                foreach (DName var in al)
                {
                    if (Program.model.modelGekko.m2.endogenous.ContainsKey(var.ToString()))
                    {
                        Program.model.modelGekko.numberOfEndo++;
                        endo.Add(var.ToString());
                    }
                }
                ...
                foreach (DName var in al)
                {
                    if (!Program.model.modelGekko.m2.endogenous.ContainsKey(var.ToString()))
                    {
                        exo.Add(var.ToString());
                    }
                    if (!Program.model.modelGekko.m2.endogenous.ContainsKey(var.ToString()) && !Program.model.modelGekko.varsJTypeAutoGenerated.ContainsKey(var) && !Program.model.modelGekko.varsDTypeAutoGenerated.ContainsKey(var) && !Program.model.modelGekko.varsZTypeAutoGenerated.ContainsKey(var))
                    {
                        Program.model.modelGekko.numberOfExo++;
                        exotrue.Add(var.ToString());
                    }
                }
