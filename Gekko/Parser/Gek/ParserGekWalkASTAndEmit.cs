    ...
    {   
        ...
        public static void WalkASTAndEmit(ASTNode node, int absoluteDepth, int relativeDepth, string textInput, W w, P p)
        {
            ...
            else if (node.Text != null && node.Text.StartsWith("ASTOPT_DATE_"))
            {
                ...
                if (node.ChildrenCount() == 0) node.Code.A("o" + Num(node) + ".opt_" + s2.ToLower() + " = GekkoTime.tSimilarToNull;" + G.NL);
        ...
        private static void WalkASTAndEmitAfter(ASTNode node, W w)
        {
            switch (node.Text)
            {
                ...
                case "ASTINTERPOLATE":
                    {
                        ...
                        node.Code.A("o" + Num(node) + ".lhs = " + node[0].Code + ";" + G.NL);
                        node.Code.A("o" + Num(node) + ".rhs = " + node[1].Code + ";" + G.NL);
                        if (node[2].ChildrenCount() > 0) node.Code.A("o" + Num(node) + ".indicator = " + node[2][0].Code + ";" + G.NL);
                        if (node[3].ChildrenCount() > 0) node.Code.A("o" + Num(node) + ".type = " + "O.ConvertToString(" + node[3][0][0].Code.ToString() + ")" + ";" + G.NL);
                        if (node[4].ChildrenCount() > 0) node.Code.A("o" + Num(node) + ".type2 = " + "O.ConvertToString(" + node[4][0][0].Code.ToString() + ")" + ";" + G.NL);
                        GetCodeFromAllChildren(node, node[5]);  //options
                        node.Code.A("o" + Num(node) + ".gekkocode = @`" + G.StripQuotes(G.ReplaceGlueSymbols(node.specialExpressionAndLabelInfo[1], true)) + "`;" + G.NL);
                        node.Code.A("o" + Num(node) + ".p = p;");
                        node.Code.A("o" + Num(node) + ".Exe();" + G.NL);
                    }
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i < node[0].ChildrenCount(); i++)
                            {
                                ...
                                if (Globals.fixForGekko4_0)
                                {
                                    ...
                                    //--> the code below makes the parallel loop fail, it would need strings(#m2) to work in Gekko 4.0
                ...
                case "ASTPROCEDUREDEF":
                    {
                        ...
                        for (int i = 0; i < numberOfOptionalParameters; i++)  //i = 0, 1 in example
                        {
                            ...
                            w.headerCs.AppendLine("return " + Globals.functionSpecialName2 + numberOfParameters + "(p, " + libraryName + ", `" + functionNameLower + "`)(smpl, p, false " + GetParametersInAList(node, numberOfParametersOverloadedVersion, 1) + " " + prompts + ");");
                            w.headerCs.AppendLine("}" + G.NL);
                            w.headerCs.AppendLine("else" + G.NL);
                            w.headerCs.AppendLine("{" + G.NL);
                            w.headerCs.AppendLine("return " + Globals.functionSpecialName2 + numberOfParameters + "(p, " + libraryName + ", `" + functionNameLower + "`)(smpl, p, false " + GetParametersInAList(node, numberOfParametersOverloadedVersion, 1) + " " + prompts2 + ");");
                            w.headerCs.AppendLine("}" + G.NL);
                ...
                case "ASTPROCEDURE":
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (node.Text == "ASTOBJECTFUNCTION" || node.Text == "ASTOBJECTFUNCTION_Q")
                                {
                                    node.Code.A(Globals.functionSpecialName2).A(numberOfArguments + 1).A("(p, ").A(libraryName).A(", `").A(functionName).A("`)(" + Globals.functionTP1Cs + "").A(", " + q).A(", " + aa1).A(", " + Globals.objFunctionPlaceholder + "").A(aa2).A(")");
                                }
                                else if (node.Text == "ASTOBJECTFUNCTIONNAKED" || node.Text == "ASTOBJECTFUNCTIONNAKED_Q")
                                {
                                    node.Code.A(Globals.functionSpecialName2).A(numberOfArguments + 1).A("(p, ").A(libraryName).A(", `").A(functionName + "").A("`)(" + Globals.functionTP1Cs + "").A(", " + q).A(", " + aa1).A(", " + Globals.objFunctionPlaceholder + "").A(aa2).A(")");
                                ...
                                {
                                    node.Code.A(Globals.functionSpecialName2).A(numberOfArguments).A("(p, ").A(libraryName).A(", `").A(functionName).A("`)(" + Globals.functionTP1Cs + "").A(", " + q).A(", " + aa1 + aa2).A(")");
                                }
                    ...
                    {
                        {
                            ...
                            node.Code.A("    o" + Num(node) + ".opt_i = GekkoTime.tSimilarToNull;");
                            node.Code.A("  }");
                            node.Code.A("  else {");
                            ...
                            node.Code.A("    o" + Num(node) + ".opt_yoy = `yes`;");
                            node.Code.A("  }");
                            node.Code.A("  else {");
                ...
                case "ASTA":
                    {
                        node.Code.A(AddOperator("a", node[0].Code.ToString(), node.Parent.Parent.Text, node));
                    }
                    break;
                case "ASTAN":
                    {
                ...
                case "ASTA3":
                    {
                        node.Code.A(AddOperator("a3", node[0].Code.ToString(), node.Parent.Parent.Text, node));
                    }
                    break;
                case "ASTA3N":
                    {
                ...
                case "ASTASSIGNMENT":
                    {
                        ...
                        else
                        {
                            ...
                            if (node.specialExpressionAndLabelInfo != null)
                            {
                                ...
                                sb.A("o" + Num(node) + ".opt_trace = @`" + G.ReplaceGlueSymbols(node.specialExpressionAndLabelInfo[1], true) + "`;" + G.NL);
                            }
...
*/
                ...
                case "ASTTABLESETVALUESELEMENT":
                    {
                        ...
                        string givenLabel = null;  //Is this used at all??? Remove in Gekko 4.0.
                        if (node.Text != "ASTTABLESETVALUESELEMENT")
                        {
                ...
                case "ASTSPLICE":
                    {                        
                        ...
                        {                            
                            node.Code.A("o" + Num(node) + ".rhs.Add(" + node[i].Code + ");" + G.NL);
                        }
                        node.Code.A("o" + Num(node) + ".gekkocode = @`" + G.StripQuotes(G.ReplaceGlueSymbols(node.specialExpressionAndLabelInfo[1], true)) + "`;" + G.NL);
                        node.Code.A("o" + Num(node) + ".Exe();" + G.NL);
                    }
                ...
                case "ASTEVAL":
                    {
                        ...
                        node.Code.A("Globals.expressionText = @`" + G.StripQuotes(G.ReplaceGlueSymbols(node.specialExpressionAndLabelInfo[1], true)) + "`;" + G.NL);
