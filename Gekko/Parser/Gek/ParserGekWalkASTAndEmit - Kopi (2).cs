        ...
        public static void WalkASTAndEmit(ASTNode node, int absoluteDepth, int relativeDepth, string textInput, W w, P p)
        {
            ...
            else
            {
                ...
                switch (node.Text)
                {
                    ...
                    case "ASTSN":
                        {
                            node.Code.A(AddPrintCode("s", node[0].Code.ToString(), node.Parent.Parent.Text, node));
                        }
                        break;
                    case "ASTBANK":
                        {                            
                    ...
                    case "ASTSD":
                        {
                            node.Code.A(AddPrintCode("sd", node[0].Code.ToString(), node.Parent.Parent.Text, node));
                        }
                        break;
                    case "ASTSDP":
                        {
                            node.Code.A(AddPrintCode("sdp", node[0].Code.ToString(), node.Parent.Parent.Text, node));
                        }
                        break;
                    case "ASTSP":
                        {
                            node.Code.A(AddPrintCode("sp", node[0].Code.ToString(), node.Parent.Parent.Text, node));
                        }
                        break;
                    case "ASTPRTROWS":
                        {
                    ...
                    case "ASTDATES":
                        {
                            ...
                            node.Code.A("o" + Num(node) + ".t1 = " + s1 + ";" + G.NL);
                            node.Code.A("o" + Num(node) + ".t2 = " + s2 + ";" + G.NL);
                        }
                        break;
                    case "ASTDOTINDEXER":
                        {
                    ...
                    case "ASTFORDATE":
                        {
                            ...
                            //node.Code.A("while (O.CompareDates("+tempName+", "+endName+", " gn(`__d" + i2 + "`), Assigns.GetDouble(" + codeStep + ") > 0d))";
                            node.Code.A("{" + G.NL);
                    ...
                    case "ASTFORSTRING":
                        {
                            ...
                            else
                            {
                                ...
                                //node.Code.A("for (int j = 0; j < " + n0 + "; j++) {" + G.NL;
                                
                                //node.Code.A("string i = " + G.NL;
                    ...
                    case "ASTGENR":                    
                        {
                            //GENR fy = ...                            
                            node.Code.A(HandleGenr(Num(node), node[0].Code.ToString(), node[1].Code.ToString(), node[2].Code.ToString(), w, null));
                        }
                        break;
                    case "ASTGENRLHSFUNCTION":
                        {
                            //GENR dlog(fy) = ...                            
                            node.Code.A(HandleGenr(Num(node), node[0].Code.ToString(), node[1].Code.ToString(), node[2].Code.ToString(), w, node[3].Text));
                        }
                        break;
                    case "ASTGENRINDEXER":
                        {
                    ...
                    case "ASTMATRIX":
                        {
                            ...
                            else
                            {
                                ...
                                //node.Code.A("O.SetMatrixData(" + node[0].Code + ", `" + nodeLeftSide.nameSimpleIdent + "`, " + node[1].Code + ");" + G.NL;
                                node.Code.A("O.SetMatrixData(new ScalarString(`" + nodeLeftSide.nameSimpleIdent + "`), " + node[1].Code + ");" + G.NL);
                            }
                        }
                        break;
                    case "ASTGENRLISTINDEXER":
                        {
                            node.Code.A(HandleGenr(Num(node), node[0].Code.ToString(), "O.GetTimeSeriesFromList(" + node[1].Code + ", " + node[2].Code + ", 1, t)", node[3].Code.ToString(), w, null));
                            //GENR #m[2] = ...
                        }
                        break;
                    ...
                    case "ASTINDEX":  //the INDEX command
                        {
                            ...
                            if (node.ChildrenCount() > 1)
                            {
                                node.Code.A("o" + Num(node) + ".name = O.GetString(" + node[1].Code + ");" + G.NL);
                            }
                            else
                            {
                                node.Code.A("o" + Num(node) + ".name = null;");  //just show the result
                            }
                            //node.Code.A("o" + O(node) + ".listItems = O.GetList(O.IndexerAlone(" + node[0].Code + "));" + G.NL;
                    ...
                    case "ASTMENUTABLE":
                        {
                            ...
                            GetCodeFromAllChildren(node);
                            if (node.Text == "ASTMENUTABLE") node.Code.A("o.menuTable = true;" + G.NL);
                    ...
                    case "ASTTABLESETVALUESELEMENT":
                        {
                            ...
                            //node.Code.A(Globals.startListIteratorCode;
                            ...
                            //node.Code.A(node.CodeSentFromSubTree;  //Contains names of timeseries used deeper down the tree
                            //node.Code.A("TimeSeries lhs = new TimeSeries(Program.options.freq, null);" + G.NL;
                            ...
                            //node.Code.A("O.Prt.HandlePrtElements(ope" + Num(node) + ", bankNumber, t, data);" + G.NL;                      
                            node.Code.A(Globals.endGekkoTimeIteratorCode);
                            //node.Code.A("if (bankNumber == 1) ope" + Num(node) + ".tsWork = lhs; else ope" + Num(node) + ".tsBase = lhs;" + G.NL;                                                     
                            node.Code.A("}" + G.NL);
                            node.Code.A("o" + Num(node) + ".prtElements.Add(ope" + Num(node) + ");" + G.NL);
                            //node.Code.A(Globals.endListIteratorCode;
                            node.Code.A("}" + G.NL);  //avoid scope collisions
                        }
                        break;
                    case "ASTOLSELEMENTS":
...
//Globals.globalGekkoTimeIterator_DO_NOT_ALTER = Globals.tNull;
                    ...
                    case "ASTUPD":
                        {
                            ...
                            //node.Code.A("int bankNumber = 1;" + G.NL;
                            ...
                            //node.Code.A("o" + Num(node) + ".lhs = O.GetTimeSeries(" + node[0].Code + ");" + G.NL;                            

                            //HandleLeftSideTimeSeries(node, wh2, bank, variable);
                            //node.Code.A(node.CodeSentFromSubTree;  //timeseries used deeper down the tree
                            node.Code.A("o" + Num(node) + ".op = " + op + ";" + G.NL);
                            node.Code.A("o" + Num(node) + ".data = new double[" + n + "];" + G.NL);
                            node.Code.A("o" + Num(node) + ".rep = new double[" + n + "];" + G.NL);
                            for (int i = 0; i < n; i++)
                            {
                                node.Code.A("o" + Num(node) + ".data[" + i + "] = (" + node[i + nn][0].Code + ").GetVal(t);" + G.NL);
                                ...
                                if (one)
                                {
                                    node.Code.A("o" + Num(node) + ".rep[" + i + "] = 1d;" + G.NL);
                                }
                                else
                                {
                                    node.Code.A("o" + Num(node) + ".rep[" + i + "] = (" + repCs + ").GetVal(t);" + G.NL);
                                }
                            }
                            node.Code.A("o" + Num(node) + ".Exe();" + G.NL);
                            //G.Writeln(node.Code);
                    ...
                    case "ASTTUPLE":
                        {
                            ...
                            else
                            {
                                ...
                                foreach (ASTNode child in node[0].ChildrenIterator())
                                {
                                    ...
                                    {
                                        ClearLocalStatementCache(w);
                                        nodeCodeTemp += HandleGenr(Num(child), child[1].Code.ToString(), child[2].Code.ToString(), tempName + ".tuple" + (number - 1), w, null);
                                    }
            ...
            if (relativeDepth == 1)
            {
                ...
                {
                    node.Code.CA("p.SetText(@`¤" + node.Line + "`);" + G.NL //so errors get line numbers
            ...
            if (node != null && absoluteDepth == 0)
            {
                ...
                foreach (ASTNode child in node.ChildrenIterator())
                {
                    //if (!w.wh.isGotoOrTarget) node.Code.A("{" + G.NL;                    
