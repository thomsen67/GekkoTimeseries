...
namespace Gekko
{
    ...
    public enum EEquationType
    {
        ...
        I,
        S,
        RevertedX,  //table variables that are run after simulation (same as RevertedY, but not technical and shown in doc systems)        
        RevertedY,  //technical, typically manual reverted J-factors               
        Z,  //these equations are ignored -- only for use in PREDICT
    ...
    {       
        ...
        {
            return eh.equationType == EEquationType.RevertedAutoGenerated || eh.equationType == EEquationType.RevertedY || eh.equationType == EEquationType.RevertedX;
        }
        ...
        private static void EmitRevertedEquations(StringBuilder code)
        {
            ...
            try
            {
                ...
                code.AppendLine("public static void revertedX(double[] b) {");
                foreach (EquationHelper eh in Program.model.modelGekko.equationsReverted)
                {
                    if (eh.equationType == EEquationType.RevertedX)
                    {
                        code.Append(eh.csCodeLhsGauss);
                        code.Append(" = ");
                        code.AppendLine(eh.csCodeRhs);
        ...
        private static void ExtractDJZAndEquationTypeFromEquationCode(EquationHelper eh, ModelGekko model, bool isModel)
        {
            ...
            if (G.Equal(code.Substring(0, 1), "x")) //FRML x... for table variables
            {
                eh.equationType = EEquationType.RevertedX;
                return;
            }
            else if (G.Equal(code.Substring(0, 1), "y"))
            {
