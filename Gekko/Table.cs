    ...
    public class Table
    {
        ...
        private static string GetStringFromNumber(Cell cell, string format, Table table)
        {
            ...
            try
            {
                ...
                {
                    G.Writeln("*** ERROR: Table number format should start with 'f', e.g. 'f10.2': " + format);
                    throw new GekkoException();
                }
                ...
                if (format4.Length != 2)
                {
                    G.Writeln("*** ERROR: Table number format should contain a '.', e.g. 'f10.2': " + format);
                    throw new GekkoException();
                }
                string f0 = format4[0];
                string f1 = format4[1];
                ...
                {
                    G.Writeln("*** ERROR: Table number format should contain numbers, e.g. 'f10.2': " + format);
                    throw new GekkoException();
                }
                int ff1 = -12345;
            ...
            {
                G.Writeln("*** ERROR: Table number format: could not parse: " + format);
                throw new GekkoException();
            }
        ...
        public void Set(int row, int col, Series tsWork, Series tsBase, string varName, GekkoTime tStart, GekkoTime tEnd, string tableGlobalPrintCode, string printcode, double scale, string format)
        {
            ...
            foreach (GekkoTime t in new GekkoTimeIterator(tStart, tEnd))
            {
                ...
                if (true)
                {
                    Program.ComputeValueForPrintPlotNew(out var1, out varPch, printcode, t, tsWork, tsBase, false, true);
                }                             
